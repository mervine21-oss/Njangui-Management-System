...
if (!$group_id) { echo '<div class="p-3">Missing group id</div>'; exit; }

$stmt = $pdo->prepare('SELECT * FROM tontine_groups WHERE id = ?');
$stmt->execute([$group_id]);
$group = $stmt->fetch();
if (!$group) { echo '<div class="p-3">Group not found.</div>'; exit; }
...
<div class="container py-3">
  <div class="row">
    <div class="col-12 col-md-8">
      <h4 class="mb-2"><?php echo htmlspecialchars($group['name'], ENT_QUOTES); ?></h4>
      ...
      <div class="mb-2"><strong>Members:</strong> <?php echo $membersCount; ?> of <?php echo $group['capacity']; ?> (<?php echo $seatsRemaining; ?> seats remaining)</div>
      <?php if ($isMember): ?>
      ...
      <?php else: ?>
        <?php if ($seatsRemaining > 0 && (!isset($group['status']) || $group['status'] === 'active')): ?>
          <button id="joinBtnModal" class="btn btn-cta" data-group-id="<?php echo $group_id; ?>">Join Group</button>
        <?php else: ?>
        ...
        <?php if ($inviteCode): ?>
          <div class="card p-3 shadow-sm mt-3" style="background: linear-gradient(135deg, rgba(243,232,255,0.4), rgba(237,233,254,0.4));">
            ...
            <div style="display: flex; gap: 0.5rem;">
              <input type="text" id="inviteCodeInputModal" value="<?php echo htmlspecialchars($inviteCode); ?>" class="form-control" readonly style="font-family: monospace; font-size: 0.9rem; background: white;">
              <button class="btn copy-invite" data-text="<?php echo htmlspecialchars($inviteCode); ?>" style="background: var(--accent-amber); border: none; color: white; padding: 0.5rem 1rem; border-radius: 8px; cursor: pointer; font-weight: 600; transition: all 0.2s;">📋 Copy</button>
            </div>
          </div>
        <?php endif; ?>
      <?php endif; ?>
